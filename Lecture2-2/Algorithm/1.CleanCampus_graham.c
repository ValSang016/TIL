...
int orientation(Point p, Point q, Point r)
{
  ...
  if (val == 0)
    return 0;               // Three points are collinear
...
int compare(const void *vp1, const void *vp2)
{
  ...
  if (o == 0)
  {
    // If the points p1 and p2 have the same orientation with respect to p0,
    // retain the one that is farther from p0.
    return (distSq(p0, *p2) > distSq(p0, *p1))? -1 : 1;
  }
  
  // If not same, decide the order based on the orientation
  return (o == 2)? 1: -1;
}
...
for (int i = 1; i < n; i++)
{
  // Skip points with the same angle as the previous point, except for (0,0)
  ...
  if (m < 3)
    return;       // If less than three points, no convex hull can be formed
  Point stack[m]; // Stack to store points of the convex hull
  int top = -1;
  stack[++top] = points[0];
  stack[++top] = points[1];
  stack[++top] = points[2];
